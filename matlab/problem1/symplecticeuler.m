function [t, y, ham] = symplecticeuler(f, y0, h, H, tfinal)
  % symplecticeuler(f, y0, h, H, tfinal)  Uses the symplectic Euler method to 
  %                                       evaluate y' = f(t, y) and keep track 
  %                                       of the Hamiltonian discrepancy where 
  %                                       y(0) = y0, h is the time step size, 
  %                                       H is the Hamiltonian, and tfinal is 
  %                                       the largest t value
  ...
  while t(end) < (tfinal - h)
    ...
    ytemp = y(:,n+1) + h * feval(f, t(n+1), y(:,n+1));
    y(1,n+1) = ytemp(1);
